...
| **R4 `bin/cognito.py -p/--password` puts a password in argv**, so it lands in shell history and is visible to `ps` while the command runs. | Medium     | Medium | Open. The generated-password path (omit `-p`) does not have this problem and is the default.                                                            |
