...
2. Listens on a configurable port (for web services)
3. Has a health check endpoint
4. Reads configuration from environment variables
...
| SSM Parameter Store | Secrets storage | (managed via `bin/manage-secrets.py`) | Yes |
