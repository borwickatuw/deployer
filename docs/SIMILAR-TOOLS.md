...
2. **Environment configurations** with per-environment instantiation (staging, production)
3. **Deploy script** (`deploy.py`) that reads TOML configs, builds Docker images, pushes to ECR, runs migrations, and deploys to ECS
4. **Shared environments** allowing multiple simple apps to share VPC, NAT Gateway, ALB, and ECS cluster
5. **Migration optimization** with hash-based skip when migrations are unchanged
6. **Supporting scripts** for Cognito access control, secrets management, and capacity reporting
...
2. **Replace OpenTofu modules** with Kubernetes manifests (Deployments, Services, Ingress)
3. **Handle migrations** via Kubernetes Job resources (Helm pre-install hooks)
4. **Different infrastructure provisioning** - EKS/GKE cluster setup, managed services integration
5. **Service mesh** considerations for internal service communication
...
2. **Shared environments** - Multiple apps can share VPC, NAT Gateway, ALB, and ECS cluster for cost savings
3. **Integrated migrations** - First-class support for running migrations as ECS tasks before deployment
4. **OpenTofu/Terraform-based** - Full infrastructure control, not locked to CloudFormation
5. **Migration optimization** - Hash-based skip when migrations haven't changed
