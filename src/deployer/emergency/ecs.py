...
def rollback_service(
    ...
    Args:
        cluster_name: Name of the ECS cluster
        service_name: Name of the service
        revisions_back: How many revisions to go back (default 1 = previous)

    Returns:
        Tuple of (new_task_def_arn, old_task_def_arn) or (None, None) on error
    """
    ...
    current_arn = state.task_definition
    family = current_arn.split("/")[-1].rsplit(":", 1)[0]

    # Get recent revisions
    revisions = list_task_definition_revisions(family, max_results=revisions_back + 2)
    if len(revisions) < revisions_back + 1:
        return None, None

    # Get the target revision
    target = revisions[revisions_back]

    # Update service
    if update_service_task_definition(cluster_name, service_name, target["arn"]):
        return target["arn"], current_arn
...
def wait_for_deployment(
    ...
    Args:
        ...
        callback: Optional callback(running, desired) called on each poll
...
def scale_all_services(
    cluster_name: str,
    multiplier: float = 1.0,
    fixed_count: int | None = None,
) -> dict[str, tuple[int, int]]:
    """Scale all services in a cluster.

    Args:
        cluster_name: Name of the ECS cluster
        multiplier: Scale current count by this multiplier
        fixed_count: If set, scale all services to this count (overrides multiplier)

    Returns:
        Dict mapping service name to (old_count, new_count)
    """
    services = get_all_services_state(cluster_name)
    result = {}

    for name, state in services.items():
        old_count = state.desired_count
        ...
        if scale_service(cluster_name, name, new_count):
            result[name] = (old_count, new_count)
