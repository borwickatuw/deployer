...
class RdsState:
    ...
    instance_id: str
    status: str
    snapshot_before: str | None = None  # Snapshot created before action
...
def list_checkpoints(environment: str | None = None) -> list[Checkpoint]:
    ...
    for filepath in checkpoint_dir.glob("emergency-*.json"):
        try:
            ...
            if environment is None or checkpoint.environment == environment:
