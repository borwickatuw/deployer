...
class StorageModule(ResourceModule):
    ...
    def validate(
        ...
        if not app_config:
            return []  # Storage not declared - not an error

        storage_type = app_config.get("type")
        if not storage_type:
            errors.append("[storage] section missing 'type' in deploy.toml")
            return errors

        if storage_type != "s3":
            errors.append(f"[storage] type '{storage_type}' not supported (only 's3')")
            return errors
        ...
        # Check env config provides each declared bucket
        if not env_config:
            errors.append("[storage] section missing from config.toml")
            return errors
