...
def is_infrastructure_service(name: str) -> bool:
...
def is_optional_service(service_config: dict) -> bool:
    """Check if a service is optional (has profiles defined)."""
    return bool(service_config.get("profiles"))
...
def _filter_app_services(services: dict) -> dict:
    ...
    return {
        ...
        if svc.get("has_build")
        and not is_infrastructure_service(name)
        and not is_optional_service(svc)
    }
...
def generate_deploy_toml(
    ...
    infra_services = [name for name in services.keys() if is_infrastructure_service(name)]
...
def _format_application_section(app: dict) -> list[str]:
    ...
    if app.get("ecr_prefix"):
        lines.append(f'ecr_prefix = "{app["ecr_prefix"]}"')
    lines.append("")
    return lines
