...
output "waf_web_acl_arn" {
  ...
  value       = var.waf_enabled && length(module.waf) > 0 ? module.waf[0].web_acl_arn : ""
}

output "waf_log_group_name" {
  description = "CloudWatch log group name for WAF logs (empty if WAF disabled)"
  value       = var.waf_enabled && length(module.waf) > 0 ? module.waf[0].log_group_name : ""
}
...
output "cloudfront_distribution_id" {
  description = "CloudFront distribution ID (empty if CloudFront disabled)"
  value       = var.cloudfront_enabled && length(aws_cloudfront_distribution.media) > 0 ? aws_cloudfront_distribution.media[0].id : ""
}

output "cloudfront_domain_name" {
  description = "CloudFront distribution domain name (empty if CloudFront disabled)"
  value       = var.cloudfront_enabled && length(aws_cloudfront_distribution.media) > 0 ? aws_cloudfront_distribution.media[0].domain_name : ""
}

output "cloudfront_custom_domain" {
  description = "Custom domain for CloudFront (if configured, empty otherwise)"
  value       = var.cloudfront_domain
}

output "cloudfront_key_id" {
  description = "CloudFront key pair ID for signed URLs (empty if not configured)"
  value       = var.cloudfront_enabled && var.cloudfront_public_key_pem != "" && length(aws_cloudfront_public_key.media) > 0 ? aws_cloudfront_public_key.media[0].id : ""
}
