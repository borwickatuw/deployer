# Database on Shared RDS Module
#
# Creates a database and users on an existing shared RDS instance.
# This enables multiple applications to share a single RDS instance
# while maintaining complete data isolation through PostgreSQL's
# permission model.
#
# Creates:
# - A DATABASE on the shared RDS instance
# - App user: DML only (SELECT, INSERT, UPDATE, DELETE)
# - Migrate user: DDL + DML (CREATE, ALTER, DROP, etc.)
#
# Uses a Lambda function to connect to PostgreSQL and create resources.
...
variable "db_name" {
  type        = string
  description = "Name of the database to create on the shared RDS instance"
}

variable "db_host" {
  type        = string
  description = "Hostname of the shared RDS instance"
}
...
variable "master_secret_arn" {
  type        = string
  description = "ARN of the Secrets Manager secret containing master credentials for the shared RDS instance"
}
...
variable "db_security_group_id" {
  type        = string
  description = "Security group ID for the shared RDS instance (Lambda will be allowed to connect)"
}
...
resource "aws_security_group" "lambda" {
  name        = "${var.name_prefix}-db-setup-lambda"
  description = "Security group for db-on-shared-rds Lambda function"
  vpc_id      = var.vpc_id
...
resource "aws_security_group_rule" "lambda_to_db" {
  ...
  description              = "Allow db-on-shared-rds Lambda to connect to PostgreSQL"
}
...
resource "aws_lambda_function" "setup_db" {
  ...
  function_name    = "${var.name_prefix}-setup-db-on-shared-rds"
...
resource "aws_lambda_invocation" "setup_db" {
  function_name = aws_lambda_function.setup_db.function_name
