...
def create_database(conn, db_name: str) -> None:
    """Create a new database on the shared RDS instance.

    Note: CREATE DATABASE cannot run inside a transaction, so we use
    autocommit mode (which pg8000.native uses by default).
    """
    # Use escaped identifier to handle special characters
    conn.run(f"CREATE DATABASE {escape_identifier(db_name)}")
...
def handler(event, context):
    ...
    try:
        ...
        if database_exists(conn_admin, db_name):
            logger.info(f"Database '{db_name}' already exists")
        else:
            create_database(conn_admin, db_name)
            db_created = True
