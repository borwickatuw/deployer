...
class TestCdnModule:
    ...
    def test_validate_cloudfront(self):
        """Test validation with valid CloudFront config."""
        module = CdnModule()
        app_config = {"type": "cloudfront"}
        env_config = {
            "domain": "d123.cloudfront.net",
            "key_id": "K123",
            "private_key_param": "/app/staging/cloudfront-key",
        }
    ...
    def test_validate_missing_key_id(self):
        """Test validation fails when key_id missing."""
        module = CdnModule()
        app_config = {"type": "cloudfront"}
        env_config = {
            "domain": "d123.cloudfront.net",
        ...
        assert any("key_id" in e for e in errors)

    def test_collect(self):
        """Test collecting CDN config."""
        module = CdnModule()
        app_config = {"type": "cloudfront"}
        env_config = {
            "domain": "d123.cloudfront.net",
        ...
        env_names = {e.name: e.value for e in output.environment}
        assert env_names["CLOUDFRONT_DOMAIN"] == "d123.cloudfront.net"
        assert env_names["CLOUDFRONT_KEY_ID"] == "K123"

        assert len(output.secrets) == 1
        assert output.secrets[0].name == "CLOUDFRONT_PRIVATE_KEY"
