...
Usage:
    ...
    python bin/ssm-secrets.py check ../app/deploy.toml myapp-staging
    ...
    python bin/ssm-secrets.py put myapp-staging SECRET_KEY --random 64

    # Set a secret with value from command line (less secure - visible in history)
    python bin/ssm-secrets.py put myapp-staging SECRET_KEY --value "my-secret-value"

    # Set a secret with value from file
    python bin/ssm-secrets.py put myapp-staging SSL_CERT --from-file /path/to/cert.pem
...
def cmd_check(args) -> int:
    ...
    if used_explicit_flag:
        ...
        print(f"     to check with just: ssm-secrets.py check {args.environment}\n")
    ...
    if missing:
        ...
        for env_var, ssm_path in missing:
            ...
            print(f"  uv run python bin/ssm-secrets.py put {args.environment} {secret_name}")

    if extra:
        print("\nExtra secrets not referenced in deploy.toml:")
        for secret_name, ssm_path in extra:
            print(f"  uv run python bin/ssm-secrets.py delete {args.environment} {secret_name}")
