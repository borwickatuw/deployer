...
def build_meta(
    ...
    return {
        ...
        "config_toml_hash": compute_hash(config_toml_content),
        "tofu_outputs_hash": compute_hash(tofu_outputs_json),
    }
...
def verify_config(environment: str, resolved_config: dict) -> bool:
    ...
    if stored_tofu_hash:
        tofu_outputs = get_all_tofu_outputs(env_path)
        current_tofu_hash = compute_hash(json.dumps(tofu_outputs, sort_keys=True))
    ...
    if stored_config_hash:
        ...
        current_config_hash = compute_hash(config_content)
...
Examples:
  ...
  python resolve-config.py myapp-staging --verify
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Verify an existing resolved config is still fresh",
    )
    parser.add_argument(
        "--verify-file",
        metavar="FILE",
    ...
    try:
        configure_aws_profile_for_environment("infra", args.environment, validate=True)
    ...
    if args.verify:
        ...
        verify_path = args.verify_file or args.output
        if not verify_path:
        ...
        log(f"Verifying resolved config for {args.environment}...")
        if verify_config(args.environment, resolved):
    ...
    try:
        log(f"Resolving config for {args.environment}...")
        resolved = resolve_config(args.environment)
